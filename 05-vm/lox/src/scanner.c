...
static TokenType checkKeyword(Scanner* scanner, size_t start, size_t length,
    ...
    size_t total = start + length;
    if ((size_t)(scanner->current - scanner->start) == total &&
...
static TokenType identifierType(Scanner* scanner) {
    switch (scanner->start[0]) {
        ...
        case 'f':
            if ((size_t)(scanner->current - scanner->start) > 1) {
...
static Token number(Scanner* scanner) {
    ...
    Token token = makeToken(scanner, TOKEN_NUMBER);
    token.literalNumber = strtod(scanner->start, NULL);
    return token;
}
